    ...
    public class Ohana
	{
		...
		public void Load_Textures(string File_Name, bool Create_DX_Texture = true)
		{
			...
			if (CLIM_Magic == "CLIM") {
				...
				for (int Offset = Base_Offset; Offset <= Base_Offset + (Entries * 0x10) - 1; Offset += 0x10) {
                    ...
                    MyTex.Name = Texture_Name;
                    MyTex.Has_Alpha = Check_Alpha(Out);
                    MyTex.Offset = Data_Offset;
                    MyTex.Format = Format;
					Model_Texture.Add(MyTex);
				}
			} else {
					...
					if (Magic == "BCH") {
						...
						while (Index < Texture_Count) {
							if (Common.Read32(Data, Description_Offset + Desc_Texture_Pointer - Texture_Description_Length) != Common.Read32(Data, Description_Offset + Desc_Texture_Pointer)) {
                                ...
                                MyTex.Name = Texture_Names[Index];
                                MyTex.Has_Alpha = Check_Alpha(Out);
                                MyTex.Offset = Texture_Data_Offset;
                                MyTex.Format = Format;
								Model_Texture.Add(MyTex);
		...
		private void Load_BCH_Textures(byte[] Data, int Count, int BCH_Offset, int Header_Offset, int Data_Offset, int Description_Offset, int Texture_Names_Offset, int BCH_Texture_Table, BCH_Version Version)
		{
			...
			for (var Index = 0; Index <= Count - 1; Index++) {
                ...
                MyTex.Name = Texture_Name;
                MyTex.Has_Alpha = Check_Alpha(Out);
                MyTex.Offset = Texture_Offset + BCH_Offset;
                MyTex.Format = Texture_Format;
				Model_Texture.Add(MyTex);
			}
		}
		...
		public void Insert_Texture(string File_Name, int LstIndex, bool Show_Warning = true)
		{
            ...
            // Sometimes a race condition messes this up
            var retryCount = 0;
            while(Img == null)
            {
                try
                {
                    Img = new Bitmap(File_Name);
                }
			...
			if ((Img.Width != Model_Texture[LstIndex].Image.Width) | (Img.Height != Model_Texture[LstIndex].Image.Height)) {
				if (Show_Warning)
					throw new InvalidOperationException("Images need to have the same resolution!");
				return;
			}
                            ...
                            for (int Tile_X = 0; Tile_X <= (Img.Width / 8) - 1; Tile_X++)
                            {
                                for (int i = 0; i <= 63; i++)
                                {
                                    ...
                                    switch (Format)
                                    {
                                        ...
                                        default:
                                            // NOTE -- if you get here, see if Convert_Texture implemented the missing format
		...
		public void Insert_OBJ(string File_Name)
		{
			...
			foreach (string ObjLine in Lines) {
				string Line = ObjLine.Trim().ToLower();
			...
			if (!Vtx_OK & Face_Index < _with18.Index.Length) {
				throw new InvalidOperationException("The inserted object have too much faces and vertices." + Environment.NewLine + "Try limiting it to the original counts.");
			} else if (Face_Index / 3 < Faces.Count) {
                throw new InvalidOperationException("The inserted object have more faces than the original one." + Environment.NewLine + "Some faces couldn't be added.");
			} else if (!Vtx_OK) {
                throw new InvalidOperationException("The inserted object have more vertices than the original one." + Environment.NewLine + "Some vertices couldn't be added.");
			}
